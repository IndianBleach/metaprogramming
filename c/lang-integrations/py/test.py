...
class _NativeStack:
    def __init__(self):
        self.stacklib = cdll.LoadLibrary("libstack.so")
        
        # value_t make_value(char* data, size_t len);
        self._make_value = self.stacklib.make_value
        self._make_value.argtypes = [c_char_p, c_int]
        self._make_value.restype = value_t

        # value_t copy_value(char* data, size_t len);
        self._copy_value = self.stacklib.copy_value
        self._copy_value.argtypes = [c_char_p, c_int]
        self._make_value.restype = value_t
...
class Stack:
    ...
    def push(self, item):
        result = self._nativeApi_._push_(
            self._handler_,
            self._nativeApi_._copy_value(item.encode('utf-8'), len(item))
        )
    ...
    def clear(self):
        self._nativeApi_._clear_(
            self._handler_,
            self._nativeApi_._freevalue_)
